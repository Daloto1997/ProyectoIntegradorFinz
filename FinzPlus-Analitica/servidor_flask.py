...
def obtener_df_metas():
    """Intenta consumir la API real. Si falla, usa simulación de fallback."""
    try:
...
def graficas_metas():
    """Genera y retorna las gráficas de Metas como imágenes base64."""
    df = limpiar_datos_metas(obtener_df_metas())
    if df.empty:
...
def graficas_deudas():
    """Genera y retorna las gráficas de Deudas como imágenes base64."""
    df = limpiar_datos_deudas(obtener_df_deudas())
    if df.empty:
